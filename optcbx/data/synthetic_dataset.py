...
@click.option('-o', '--output', required=True, type=click.Path(file_okay=False))
...
def main(input_path: str, output: str, epochs: int, mix_from: int,
         min_portraits: int, max_portraits: int):
    ...
    for epoch in range(epochs):
        for annotation_path in tqdm.tqdm(annotations_paths,
                                         desc=f'Epoch {epoch}'):
            mix_with = [
                random.choice(annotations_paths) for _ in range(mix_from)
            ]
            mixers = _pick_mixers(base_path, mix_with, min_portraits,
                                  max_portraits)
...
def _pick_mixers(base_path: Path, mix_with: List[Path], min_portraits: int,
    ...
    n = random.randint(min_portraits, min(max_portratits, indices.shape[0]))
    ...
    return [
        im[r[1]:r[3], r[0]:r[2]] for im, r in zip(selected_ims, selected_rects)
    ]
...
def _paste_mixers(base_path: Path, annotation_path: Path,
                  mixers: List[np.ndarray]) -> np.ndarray:
    im, rects = _load_example(base_path, annotation_path)
    selected_rects_idx = _randint(0,
                                  rects.shape[0],
